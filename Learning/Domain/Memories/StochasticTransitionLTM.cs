// ------------------------------------------
// StochasticTransitionLTM.cs, Learning
// 
...
using Action = Learning.Domain.Actions.Action;
...
namespace Learning.Domain.Memories
{
    ...
    public class StochasticTransitionLTM : LongTermMemory
    {
        ...
        {
            return string.Format("{0}oStates-{1}noStates-{2}actions",
                this.observableStates.Count, this.nonObservableStates.Count, this.Agent.Actions.Count);
        }
        ...
        protected virtual void CreateActions()
        {
            ...
            for (var i = 0; i < numActions; i++)
            {
                ...
                this.Agent.Actions.Add(actionID, new Action(actionID, this.Agent));
            }
        }
        ...
        protected virtual void DeserializeActionsXML(XmlElement element)
        {
            ...
            {
                var action = new Action("action", null);
        ...
        protected virtual void SerializeActionsXML(XmlElement element)
        {
            ...
            element.AppendChild(childElement);
            foreach (Action action in this.Agent.Actions.Values)
            {
        ...
        protected virtual void SerializeRewardsXML(XmlElement element)
        {
            ...
            foreach (var state in this.rewardTable.Keys)
                foreach (Action action in this.rewardTable[state].Keys)
                {
        ...
        protected virtual void SerializeTransitionsXML(XmlElement element)
        {
            ...
            foreach (var initialState in this.stateTransitionProbabilities.Keys)
                foreach (Action action in this.stateTransitionProbabilities[initialState].Keys)
